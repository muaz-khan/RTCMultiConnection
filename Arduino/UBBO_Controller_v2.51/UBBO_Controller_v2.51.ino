...
int    iNbVar= 0;
byte   byCmd = 0x00;
byte   byPosition = 0x00;
byte   byPreviousCmd = 0x00;
...
FastIRReceiver infrared_dock_sensor_right;
FastIRReceiver infrared_dock_sensor_left;
FastIRReceiver infrared_dock_sensor_center;
...
void setup()
{
    ...
    // init IR
    // FastIRReceiver::codeList[0] = IR_CODE_LEFT;
    // FastIRReceiver::codeList[1] = IR_CODE_RIGHT;
    //
    // infrared_dock_sensor_left.pin = IR_LEFT_PIN;
    // FastIRReceiver::receiverList[0] = &infrared_dock_sensor_left;
    //
    // infrared_dock_sensor_right.pin = IR_RIGHT_PIN;
    // FastIRReceiver::receiverList[1] = &infrared_dock_sensor_right;

    // infrared_dock_sensor_center.pin = IR_CENTER_PIN;
    // FastIRReceiver::receiverList[2] = &infrared_dock_sensor_center;
    ...
    // pinMode(FRONT_PROX_SENSOR_PIN,INPUT);
    // pinMode(RIGHT_PROX_SENSOR_PIN,INPUT);
    // pinMode(BACK_PROX_SENSOR_PIN,INPUT);
    // pinMode(LEFT_PROX_SENSOR_PIN,INPUT);
    // Set pull_up resistors
    // if(!FRONT_PROX_SENSOR_IS_ANALOG)digitalWrite(FRONT_PROX_SENSOR_PIN,HIGH);
    // if(!LEFT_PROX_SENSOR_IS_ANALOG)digitalWrite(LEFT_PROX_SENSOR_PIN,HIGH);
    // if(!BACK_PROX_SENSOR_IS_ANALOG)digitalWrite(BACK_PROX_SENSOR_PIN,HIGH);
    // if(!RIGHT_PROX_SENSOR_IS_ANALOG)digitalWrite(RIGHT_PROX_SENSOR_PIN,HIGH);
    ...
    alarmDockStationVoltage();
}

void loop()
{

    // check dockStation charging power
    checkDockStationCharger();

    //Check proximy sensors and send serial info (Add-on requested by Orange Labs 05/11/2015)
    ...
    while(true){
      if (byCmd != 0)
      {
          if (byPreviousCmd != byCmd)
          {
          }
          ulTimeStart = millis();
          bTimeoutDetected = false;
          //update byPreviousData value
          switch (byCmd)
          {
              //Go forward
              case GO_FORWARD:
                  if (iMessageSize == 1)
                  {
                      ...
                      dbg_println("bad \"move robot\" message format");
                  break;
                  // Go back
              case GO_BACKWARD:
                  if (iMessageSize == 1)
              ...
              case STOP:
                  ...
                  // Translate left
              case TRANSLATE_LEFT:
                  // Check left presence sensor
                //   if ((!bProximityDetection) || (getProximitySensorStatus(LEFT_PROX_SENSOR_PIN) == 1))
                //   {
                      Ubbo.doMovement(Omni4WD::STAT_TRANSLATE_LEFT, iSpeedTranslate);
                      //dbg_println("translate left");
                      //update Previous values
                      byPreviousCmd = byCmd;
                //   }
                //   else
                //       dbg_println("Forbidden movement! Obstacle left");
                  //
                //   break;
                  // Translate right
              case TRANSLATE_RIGHT:
                  //Check right presence sensor
                //   if ((!bProximityDetection) || (getProximitySensorStatus(RIGHT_PROX_SENSOR_PIN) == 1))
                //   {
                      Ubbo.doMovement(Omni4WD::STAT_TRANSLATE_RIGHT, iSpeedTranslate);
                      //dbg_println("translate right");
                      //update Previous values
                      byPreviousCmd = byCmd;
                //   }
                //   else
                //       dbg_println("Forbidden movement! Obstacle right");
                  break;
              ...
              case MOVE_TABLET:
                  ...
                  break;
              case HEART_BEAT:
                  //dbg_println("Heart Beat");
                  break;
                  // Version status
              case VERSION_STATUS:
                  //dbg_println("Version Status");
                  sendVersionStatus();
                  break;
                  // Battery status
              case BATTERY_STATUS:
                  //dbg_println("Battery Status");
                  sendBatteryStatus();
                  break;
                  // Go Back dockstation
              case GO_BACK_DOCKSTATION:
                  backToDockStation();
                  continue;
                  // Other
              default:
