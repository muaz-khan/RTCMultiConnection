...
! function() {
    ...
    function addEvent(element, eventType, callback) {
        if (!(eventType.split(" ").length > 1)) return element.addEventListener ? (element.addEventListener(eventType, callback, !1), !0) : element.attachEvent ? element.attachEvent("on" + eventType, callback) : (element["on" + eventType] = callback, this);
        for (var events = eventType.split(" "), i = 0; i < events.length; i++) addEvent(element, events[i], callback)
    }
    ...
    function drawArrow(mx, my, lx, ly, options) {
        function getOptions(opt) {
            return opt = opt || {}, [opt.lineWidth || 2, opt.strokeStyle || "#6c96c8", opt.fillStyle || "rgba(0,0,0,0)", opt.globalAlpha || 1, opt.globalCompositeOperation || "source-over", opt.lineCap || "round", opt.lineJoin || "round", opt.font || '15px "Arial"']
        }

        function handleOptions(opt, isNoFillStroke) {
            opt = opt || getOptions(), context.globalAlpha = opt[3], context.globalCompositeOperation = opt[4], context.lineCap = opt[5], context.lineJoin = opt[6], context.lineWidth = opt[0], context.strokeStyle = opt[1], context.fillStyle = opt[2], context.font = opt[7], isNoFillStroke || (context.stroke(), context.fill())
        }
        var arrowSize = 10,
    ...
    function copy() {
        endLastPath(), dragHelper.global.startingIndex = 0, find("copy-last").checked ? (copiedStuff = points[points.length - 1], setSelection(find("drag-last-path"), "DragLastPath")) : (copiedStuff = points, setSelection(find("drag-all-paths"), "DragAllPaths"))
    }

    function paste() {
        endLastPath(), dragHelper.global.startingIndex = 0, find("copy-last").checked ? (points[points.length] = copiedStuff, dragHelper.global = {
            prevX: 0,
            prevY: 0,
            startingIndex: points.length - 1
        }, dragHelper.dragAllPaths(0, 0), setSelection(find("drag-last-path"), "DragLastPath")) : (dragHelper.global.startingIndex = points.length, points = points.concat(copiedStuff), setSelection(find("drag-all-paths"), "DragAllPaths"))
    }
    ...
    function setTemporaryLine() {
        ...
        points.push(arr), drawHelper.redraw(), setTimeout(function() {
            setSelection(document.getElementById("line"), "Line")
        }, 1e3), setTimeout(setDefaultSelectedIcon, 2e3)
    }
    ...
    function onkeypress(e) {
        null != e.which || null == e.charCode && null == e.keyCode || (e.which = null != e.charCode ? e.charCode : e.keyCode), keyCode = e.which || e.keyCode || 0, /[a-zA-Z0-9-_ !?|\/'",.=:;(){}\[\]`~@#$%^&*+-]/.test(String.fromCharCode(keyCode)) && textHandler.writeText(String.fromCharCode(keyCode))
    }
    ...
    var isControlKeyPressed, is = {
        ...
        common = {
            ...
            getPoint: function(pointToCompare, compareWith, prefix) {
                return pointToCompare = pointToCompare > compareWith ? prefix + " + " + (pointToCompare - compareWith) : pointToCompare < compareWith ? prefix + " - " + (compareWith - pointToCompare) : prefix
            },
            ...
            relativeShortened: function(toFixed, getPoint) {
                var point, p, i = 0,
                    ...
                    x = 0,
                    y = 0;
                for (i; i < length; i++) p = points[i], point = p[1], 0 === i && (x = point[0], y = point[1]), "text" === p[0] && (x = point[1], y = point[2]), "pencil" === p[0] && (output += this.shortenHelper(p[0], [getPoint(point[0], x, "x"), getPoint(point[1], y, "y"), getPoint(point[2], x, "x"), getPoint(point[3], y, "y")], p[2])), "marker" === p[0] && (output += this.shortenHelper(p[0], [getPoint(point[0], x, "x"), getPoint(point[1], y, "y"), getPoint(point[2], x, "x"), getPoint(point[3], y, "y")], p[2])), "eraser" === p[0] && (output += this.shortenHelper(p[0], [getPoint(point[0], x, "x"), getPoint(point[1], y, "y"), getPoint(point[2], x, "x"), getPoint(point[3], y, "y")], p[2])), "line" === p[0] && (output += this.shortenHelper(p[0], [getPoint(point[0], x, "x"), getPoint(point[1], y, "y"), getPoint(point[2], x, "x"), getPoint(point[3], y, "y")], p[2])), "arrow" === p[0] && (output += this.shortenHelper(p[0], [getPoint(point[0], x, "x"), getPoint(point[1], y, "y"), getPoint(point[2], x, "x"), getPoint(point[3], y, "y")], p[2])), "text" === p[0] && (output += this.shortenHelper(p[0], [point[0], getPoint(point[1], x, "x"), getPoint(point[2], y, "y")], p[2])), "arc" === p[0] && (output += this.shortenHelper(p[0], [getPoint(point[0], x, "x"), getPoint(point[1], y, "y"), point[2], point[3], point[4]], p[2])), "rect" === p[0] && (output += this.shortenHelper(p[0], [getPoint(point[0], x, "x"), getPoint(point[1], y, "y"), getPoint(point[2], x, "x"), getPoint(point[3], y, "y")], p[2])), "quadratic" === p[0] && (output += this.shortenHelper(p[0], [getPoint(point[0], x, "x"), getPoint(point[1], y, "y"), getPoint(point[2], x, "x"), getPoint(point[3], y, "y"), getPoint(point[4], x, "x"), getPoint(point[5], y, "y")], p[2])), "bezier" === p[0] && (output += this.shortenHelper(p[0], [getPoint(point[0], x, "x"), getPoint(point[1], y, "y"), getPoint(point[2], x, "x"), getPoint(point[3], y, "y"), getPoint(point[4], x, "x"), getPoint(point[5], y, "y"), getPoint(point[6], x, "x"), getPoint(point[7], y, "y")], p[2]));
            ...
            strokeOrFill: function(p) {
                return this.prevProps && this.prevProps === p.join(",") ? "strokeOrFill();" : (this.prevProps = p.join(","), "strokeOrFill('" + p.join("', '") + "');")
            },
            prevProps: null,
            shortenHelper: function(name, p1, p2) {
                var result = "['" + name + "', [" + p1.join(", ") + "]";
                return this.prevProps && this.prevProps === p2.join(",") || (this.prevProps = p2.join(","), result += ", ['" + p2.join("', '") + "']"), result + "], "
            }
        },
        copiedStuff = [],
        drawHelper = {
            redraw: function() {
                ...
                for (i = 0; i < length; i++)(point = points[i]) && point.length && this[point[0]] && this[point[0]](context, point[1], point[2])
            },
            getOptions: function(opt) {
                return opt = opt || {}, [opt.lineWidth || lineWidth, opt.strokeStyle || strokeStyle, opt.fillStyle || fillStyle, opt.globalAlpha || globalAlpha, opt.globalCompositeOperation || globalCompositeOperation, opt.lineCap || lineCap, opt.lineJoin || lineJoin, opt.font || font]
            },
            handleOptions: function(context, opt, isNoFillStroke) {
                opt = opt || this.getOptions(), context.globalAlpha = opt[3], context.globalCompositeOperation = opt[4], context.lineCap = opt[5], context.lineJoin = opt[6], context.lineWidth = opt[0], context.strokeStyle = opt[1], context.fillStyle = opt[2], context.font = opt[7], isNoFillStroke || (context.stroke(), context.fill())
            },
            line: function(context, point, options) { // 확인!
                context.beginPath(), context.moveTo(point[0], point[1]), context.lineTo(point[2], point[3]), this.handleOptions(context, options)
            },
            ...
            text: function(context, point, options) {
                this.handleOptions(context, options), context.fillStyle = textHandler.getFillColor(options[2]), context.fillText(point[0].substr(1, point[0].length - 2), point[1], point[2])
            },
            arc: function(context, point, options) {
                context.beginPath(), context.arc(point[0], point[1], point[2], point[3], 0, point[4]), this.handleOptions(context, options)
            },
            ...
            quadratic: function(context, point, options) {
                context.beginPath(), context.moveTo(point[0], point[1]), context.quadraticCurveTo(point[2], point[3], point[4], point[5]), this.handleOptions(context, options)
            },
            bezier: function(context, point, options) {
                context.beginPath(), context.moveTo(point[0], point[1]), context.bezierCurveTo(point[2], point[3], point[4], point[5], point[6], point[7]), this.handleOptions(context, options)
            }
        },
        dragHelper = {
            global: {
            ...
            init: function() {
                if (points.length) {
                    var p = points[points.length - 1],
                        point = p[1];
                    this.global.ismousedown ? tempContext.fillStyle = "rgba(255,85 ,154,.9)" : tempContext.fillStyle = "rgba(255,85 ,154,.4)", "quadratic" === p[0] && (tempContext.beginPath(), tempContext.arc(point[0], point[1], 10, 2 * Math.PI, 0, !1), tempContext.arc(point[2], point[3], 10, 2 * Math.PI, 0, !1), tempContext.arc(point[4], point[5], 10, 2 * Math.PI, 0, !1), tempContext.fill()), "bezier" === p[0] && (tempContext.beginPath(), tempContext.arc(point[0], point[1], 10, 2 * Math.PI, 0, !1), tempContext.arc(point[2], point[3], 10, 2 * Math.PI, 0, !1), tempContext.arc(point[4], point[5], 10, 2 * Math.PI, 0, !1), tempContext.arc(point[6], point[7], 10, 2 * Math.PI, 0, !1), tempContext.fill()), "line" === p[0] && (tempContext.beginPath(), tempContext.arc(point[0], point[1], 10, 2 * Math.PI, 0, !1), tempContext.arc(point[2], point[3], 10, 2 * Math.PI, 0, !1), tempContext.fill()), "arrow" === p[0] && (tempContext.beginPath(), tempContext.arc(point[0], point[1], 10, 2 * Math.PI, 0, !1), tempContext.arc(point[2], point[3], 10, 2 * Math.PI, 0, !1), tempContext.fill()), "text" === p[0] && (tempContext.font = "15px Verdana", tempContext.fillText(point[0], point[1], point[2])), "rect" === p[0] && (tempContext.beginPath(), tempContext.arc(point[0], point[1], 10, 2 * Math.PI, 0, !1), tempContext.fill(), tempContext.beginPath(), tempContext.arc(point[0] + point[2], point[1], 10, 2 * Math.PI, 0, !1), tempContext.fill(), tempContext.beginPath(), tempContext.arc(point[0], point[1] + point[3], 10, 2 * Math.PI, 0, !1), tempContext.fill(), tempContext.beginPath(), tempContext.arc(point[0] + point[2], point[1] + point[3], 10, 2 * Math.PI, 0, !1), tempContext.fill()), "image" === p[0] && (tempContext.beginPath(), tempContext.arc(point[1], point[2], 10, 2 * Math.PI, 0, !1), tempContext.fill(), tempContext.beginPath(), tempContext.arc(point[1] + point[3], point[2], 10, 2 * Math.PI, 0, !1), tempContext.fill(), tempContext.beginPath(), tempContext.arc(point[1], point[2] + point[4], 10, 2 * Math.PI, 0, !1), tempContext.fill(), tempContext.beginPath(), tempContext.arc(point[1] + point[3], point[2] + point[4], 10, 2 * Math.PI, 0, !1), tempContext.fill()), "pdf" === p[0] && (tempContext.beginPath(), tempContext.arc(point[1], point[2], 10, 2 * Math.PI, 0, !1), tempContext.fill(), tempContext.beginPath(), tempContext.arc(point[1] + point[3], point[2], 10, 2 * Math.PI, 0, !1), tempContext.fill(), tempContext.beginPath(), tempContext.arc(point[1], point[2] + point[4], 10, 2 * Math.PI, 0, !1), tempContext.fill(), tempContext.beginPath(), tempContext.arc(point[1] + point[3], point[2] + point[4], 10, 2 * Math.PI, 0, !1), tempContext.fill())
                }
            },
        ...
        pencilHandler = {
            ...
            mousedown: function(e) {
                var x = e.pageX - canvas.offsetLeft,
                    y = e.pageY - canvas.offsetTop,
                    t = this;
                t.prevX = x, t.prevY = y, t.ismousedown = !0, tempContext.lineCap = "round", pencilDrawHelper.line(tempContext, [t.prevX, t.prevY, x, y]), points[points.length] = ["line", [t.prevX, t.prevY, x, y], pencilDrawHelper.getOptions()], t.prevX = x, t.prevY = y
            },
            mouseup: function(e) {
                this.ismousedown = !1
            },
            mousemove: function(e) {
                var x = e.pageX - canvas.offsetLeft,
                    y = e.pageY - canvas.offsetTop,
                    t = this;
                t.ismousedown && (tempContext.lineCap = "round", pencilDrawHelper.line(tempContext, [t.prevX, t.prevY, x, y]), points[points.length] = ["line", [t.prevX, t.prevY, x, y], pencilDrawHelper.getOptions()], t.prevX = x, t.prevY = y)
            }
        },
    ...
    markerDrawHelper.getOptions = function() {
        return [markerLineWidth, markerStrokeStyle, fillStyle, .7, globalCompositeOperation, lineCap, lineJoin, font]
    };
    var eraserHandler = {
            ismousedown: !1,
            prevX: 0,
        ...
        textHandler = {
            ...
            onShapeSelected: function() {
                tempContext.canvas.style.cursor = "text", this.x = this.y = this.pageX = this.pageY = 0, this.text = ""
            },
            onShapeUnSelected: function() {
                this.text = "", this.showOrHideTextTools("hide"), tempContext.canvas.style.cursor = "default", void 0 !== this.blinkCursorInterval && clearInterval(this.blinkCursorInterval)
            },
            getFillColor: function(color) {
                return color = (color || fillStyle).toLowerCase(), "rgba(255, 255, 255, 0)" == color || "transparent" == color || "white" === color ? "black" : color
            },
            ...
            blinkCursor: function() {
                textHandler.index++, textHandler.index % 2 == 0 ? textHandler.fillText(textHandler.text + "|") : textHandler.fillText(textHandler.text)
            },
            getOptions: function() {
                var options = {
                ...
                return font = options.font, options
            },
            ...
            mousedown: function(e) {
                is.isText && (textHandler.text.length && this.appendPoints(), textHandler.x = textHandler.y = 0, textHandler.text = "", textHandler.pageX = e.pageX, textHandler.pageY = e.pageY, textHandler.x = e.pageX - canvas.offsetLeft - 5, textHandler.y = e.pageY - canvas.offsetTop + 10, void 0 !== textHandler.blinkCursorInterval && clearInterval(textHandler.blinkCursorInterval), textHandler.blinkCursor(), textHandler.blinkCursorInterval = setInterval(textHandler.blinkCursor, 700), this.showTextTools())
            },
            mouseup: function(e) {},
            mousemove: function(e) {},
            showOrHideTextTools: function(show) {
                "hide" === show ? this.lastFillStyle.length && (fillStyle = this.lastFillStyle, this.lastFillStyle = "") : this.lastFillStyle.length || (this.lastFillStyle = fillStyle, fillStyle = "black"), this.fontFamilyBox.style.display = "show" == show ? "block" : "none", this.fontSizeBox.style.display = "show" == show ? "block" : "none", this.fontSizeBox.style.left = this.x + "px", this.fontFamilyBox.style.left = this.fontSizeBox.clientWidth + this.x + "px", this.fontSizeBox.style.top = this.y + "px", this.fontFamilyBox.style.top = this.y + "px"
            },
            showTextTools: function() {
                this.fontFamilyBox && this.fontSizeBox && (this.unselectAllFontFamilies(), this.unselectAllFontSizes(), this.showOrHideTextTools("show"), this.eachFontFamily(function(child) {
                    child.onclick = function(e) {
                        e.preventDefault(), textHandler.showOrHideTextTools("hide"), textHandler.selectedFontFamily = this.innerHTML, this.className = "font-family-selected"
                    }, child.style.fontFamily = child.innerHTML
                }), this.eachFontSize(function(child) {
                    child.onclick = function(e) {
                        e.preventDefault(), textHandler.showOrHideTextTools("hide"), textHandler.selectedFontSize = this.innerHTML, this.className = "font-family-selected"
                    }
                }))
            },
            eachFontFamily: function(callback) {
                for (var childs = this.fontFamilyBox.querySelectorAll("li"), i = 0; i < childs.length; i++) callback(childs[i])
            },
            unselectAllFontFamilies: function() {
                this.eachFontFamily(function(child) {
                    child.className = "", child.innerHTML === textHandler.selectedFontFamily && (child.className = "font-family-selected")
                })
            },
            eachFontSize: function(callback) {
                for (var childs = this.fontSizeBox.querySelectorAll("li"), i = 0; i < childs.length; i++) callback(childs[i])
            },
            unselectAllFontSizes: function() {
                this.eachFontSize(function(child) {
                    child.className = "", child.innerHTML === textHandler.selectedFontSize && (child.className = "font-size-selected")
                })
            },
        ...
        arcHandler = {
            ...
            fixAllPoints: function() {
                for (var toFixed = this.toFixed, i = 0; i < points.length; i++) {
                    var point, p = points[i];
                    "arc" === p[0] && (point = p[1], points[i] = ["arc", [toFixed(point[0]), toFixed(point[1]), toFixed(point[2]), toFixed(point[3]), point[4]], p[2]])
                }
            },
            init: function() {
    ...
    var lineHandler = {
        ...
        zoomHandler = {
            ...
            apply: function() {
                tempContext.scale(this.scale, this.scale), context.scale(this.scale, this.scale), drawHelper.redraw()
            },
            icons: {
                up: function(ctx) {
                    ctx.font = "22px Verdana", ctx.strokeText("+", 10, 30)
                },
                down: function(ctx) {
                    ctx.font = "22px Verdana", ctx.strokeText("-", 15, 30)
                }
            }
        },
        FileSelector = function() {
            function selectFile(callback, multiple, accept) {
                ...
                file.type = "file", multiple && (file.multiple = !0), file.accept = accept || "image/*", file.onchange = function() {
        ...
        pdfHandler = {
            ...
            getPage: function(pageNumber, callback) {
                if (pageNumber = parseInt(pageNumber) || 1, !pdfHandler.pdf) return pdfjsLib.disableWorker = !1, void pdfjsLib.getDocument(pdfHandler.lastPdfURL).then(function(pdf) {
                    pdfHandler.pdf = pdf, pdfHandler.getPage(pageNumber, callback)
                });
                ...
                pdf.getPage(pageNumber).then(function(page) {
                    ...
                    !0 === pdfHandler.removeWhiteBackground && (renderContext.background = "rgba(0,0,0,0)"), page.render(renderContext).then(function() {
                        if (!0 === pdfHandler.removeWhiteBackground) {
                            for (var imgd = ctx.getImageData(0, 0, cav.width, cav.height), pix = imgd.data, newColor = {
                                    r: 0,
                                    g: 0,
                                    b: 0,
                                    a: 0
            ...
            load: function(lastPdfURL) {
                pdfHandler.lastPdfURL = lastPdfURL, pdfHandler.getPage(parseInt(pdfHandler.pdfPagesList.value || 1), function(lastPage, width, height, numPages) {
                    ...
                    pdfHandler.pdfPagesList.onchange = function() {
                        ...
                        pdfHandler.pdfPagesList.selectedIndex++, pdfHandler.pdfPagesList.onchange()
    ...
    if (params.tools) try {
            ...
            tools = t
        } catch (e) {}!0 === tools.code && (document.querySelector(".preview-panel").style.display = "block"), is.set(window.selectedIcon), window.addEventListener("load", function() {
            setDefaultSelectedIcon()
        }, !1),
        function() {
            function getContext(id) {
                var context = find(id).getContext("2d");
                return context.lineWidth = 2, context.strokeStyle = "#6c96c8", context
            }
                    ...
                    if ("pdf-icon" === this.id) {
                        ...
                        selector.selectSingleFile(function(file) {
                            function onGettingPdf() {
                                var reader = new FileReader;
                                reader.onload = function(event) {
                                    pdfHandler.pdf = null, pdfHandler.load(event.target.result)
                                }, reader.readAsDataURL(file)
                            }
                            file && onGettingPdf()
            ...
            function decorateDragLastPath() {
                var context = getContext("drag-last-path"),
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "DragLastPath")
            ...
            function decorateDragAllPaths() {
                var context = getContext("drag-all-paths"),
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "DragAllPaths")
            ...
            function decorateLine() {
                var context = getContext("line"),
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "Line")
            ...
            function decorateUndo() {
                var context = getContext("undo"),
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), document.querySelector("#undo").onclick = function() {
                        points.length && (points.length = points.length - 1, drawHelper.redraw()), syncPoints(!0)
                    }
            ...
            function decorateArrow() {
                var context = getContext("arrow"),
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "Arrow")
            ...
            function decoratePencil() {
                ...
                var colors = [
                    ...
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "Pencil")
                ...
                pencilStrokeStyle = hexToRGBA(fillStyleText.value, alpha), pencilSelectedColor.style.backgroundColor = pencilSelectedColor2.style.backgroundColor = "#" + fillStyleText.value, colors.forEach(function(colorRow) {
                    ...
                    }), row += "</tr>", pencilColorsList.innerHTML += row
                    ...
                    hideContainers(), pencilContainer.style.display = "block", pencilContainer.style.top = canvas.offsetTop + 1 + "px", pencilContainer.style.left = canvas.offsetLeft + canvas.clientWidth + "px", fillStyleText.focus()
                }), addEvent(btnPencilDone, "click", function() {
                    pencilContainer.style.display = "none", pencilColorContainer.style.display = "none", pencilLineWidth = strokeStyleText.value, pencilStrokeStyle = hexToRGBA(fillStyleText.value, alpha)
            ...
            function decorateMarker() {
                ...
                var colors = [
                    ...
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "Marker")
                ...
                markerStrokeStyle = hexToRGBA(fillStyleText.value, alpha), markerSelectedColor.style.backgroundColor = markerSelectedColor2.style.backgroundColor = "#" + fillStyleText.value, colors.forEach(function(colorRow) {
                    ...
                    colorRow.forEach(function(color) {
                        row += '<td style="background-color:#' + color + '" data-color="' + color + '"></td>'
                    }), row += "</tr>", markerColorsList.innerHTML += row
                    ...
                    markerContainer.style.display = "none", markerColorContainer.style.display = "none", markerLineWidth = strokeStyleText.value, markerStrokeStyle = hexToRGBA(fillStyleText.value, alpha)
            ...
            function decorateEraser() {
                var context = getContext("eraser-icon"),
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "Eraser")
            ...
            function decorateText() {
                var context = getContext("text-icon"),
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "Text")
            ...
            function decorateImage() {
                var context = getContext("image-icon"),
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "Image")
            ...
            function decoratePDF() {
                var context = getContext("pdf-icon"),
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "Pdf")
            ...
            function decorateArc() {
                var context = getContext("arc"),
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "Arc")
            ...
            function decorateRect() {
                var context = getContext("rectangle"),
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "Rectangle")
            ...
            function decorateQuadratic() {
                var context = getContext("quadratic-curve"),
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "QuadraticCurve")
            ...
            function decorateBezier() {
                var context = getContext("bezier-curve"),
                    image = new Image;
                image.onload = function() {
                    context.drawImage(image, 4, 4, 32, 32), bindEvent(context, "Bezier")
            ...
            function decorateColors() {
                ...
                var colorsContainer = find("colors-container"),
                    ...
                    canvas = (document.getElementsByTagName("h1")[0], context.canvas);
                addEvent(canvas, "click", function() {
                    hideContainers(), colorsContainer.style.display = "block", colorsContainer.style.top = canvas.offsetTop + 1 + "px", colorsContainer.style.left = canvas.offsetLeft + canvas.clientWidth + "px", strokeStyleText.focus()
                }), addEvent(btnColorsDone, "click", function() {
                    colorsContainer.style.display = "none", strokeStyle = strokeStyleText.value, fillStyle = fillStyleText.value
                })
            }
            ...
            function decorateAdditionalOptions() {
                ...
                addEvent(canvas, "click", function() {
                    hideContainers(), additionalContainer.style.display = "block", additionalContainer.style.top = canvas.offsetTop + 1 + "px", additionalContainer.style.left = canvas.offsetLeft + canvas.clientWidth + "px"
                }), addEvent(btnAdditionalClose, "click", function() {
                    additionalContainer.style.display = "none", globalAlpha = globalAlphaSelect.value, globalCompositeOperation = globalCompositeOperationSelect.value, lineCap = lineCapSelect.value, lineJoin = lineJoinSelect.value
                })
            }

            function btnDesignerPreviewClicked() {
                codeText.parentNode.style.display = "none", optionsContainer.style.display = "none", hideContainers(), endLastPath()
            }

            function btnCodePreviewClicked() {
                codeText.parentNode.style.display = "block", optionsContainer.style.display = "block", codeText.focus(), common.updateTextArea(), setHeightForCodeAndOptionsContainer(), hideContainers(), endLastPath()
            }

            function setHeightForCodeAndOptionsContainer() {
                codeText.style.width = innerWidth - optionsContainer.clientWidth - 30 + "px", codeText.style.height = innerHeight - 40 + "px", codeText.style.marginLeft = optionsContainer.clientWidth + "px", optionsContainer.style.height = innerHeight + "px"
            }
            var cache = {},
            ...
            window.selectBtn = function(btn, isSkipWebRTCMessage) {
                codePreview.className = designPreview.className = "", btn == designPreview ? designPreview.className = "preview-selected" : codePreview.className = "preview-selected", !isSkipWebRTCMessage && window.connection && connection.numberOfConnectedUsers >= 1 ? connection.send({
                    btnSelected: btn.id
                }) : btn == designPreview ? btnDesignerPreviewClicked() : btnCodePreviewClicked()
            }, addEvent(designPreview, "click", function() {
                selectBtn(designPreview), btnDesignerPreviewClicked()
            }), addEvent(codePreview, "click", function() {
                selectBtn(codePreview), btnCodePreviewClicked()
            });
    ...
    addEvent(canvas, isTouch ? "touchstart mousedown" : "mousedown", function(e) {
        isTouch && (e = e.pageX ? e : e.touches.length ? e.touches[0] : {
            pageX: 0,
            pageY: 0
        });
        var cache = is;
        ...
        !isTouch || e && "pageX" in e || (e = e && e.touches && e.touches.length ? e.touches[0] : e && e.changedTouches && e.changedTouches.length ? e.changedTouches[0] : {
            pageX: 0,
            pageY: 0
        });
        var cache = is;
    ...
    var test = {
        ...
        loadVideo : function(video, context,width, height){
            ...
            drawHelper.redraw()
            setTimeout( test.loadVideo, 50, video, context,width, height );
        }
    }
    // 추가
    ...
    window.addEventListener("message", function(event) {
        if (event.data) {
            if (uid || (uid = event.data.uid), event.data.captureStream) return void webrtcHandler.createOffer(function(sdp) {
            ...
            if(event.data.backgroundVideo){
                ...
                console.log("부모객체는 찾을수있냐? ",window.parent.document.getElementById(video))
                test.drawVideoToCanvas( video, context, 872, 670 )
            } 
            if(event.data.removeMainVideo){
                if(!isVideoBackGround) return;
                isVideoBackGround = false;
                setTimeout( drawHelper.redraw(), 60);
            }
            ...
            if (event.data.undo && points.length) {
                ...
                if (index.numberOfLastShapes) {
                    ...
                    return drawHelper.redraw(), void syncPoints(!0)
                }
                if (-1 === index) return points.length = points.length - 1, drawHelper.redraw(), void syncPoints(!0);
                if (points[index]) {
    ...
    var webrtcHandler = {
        createOffer: function(callback) {
            ...
            captureStream.getTracks().forEach(function(track) {
                ...
                event && !event.candidate && callback({
        ...
        setRemoteDescription: function(sdp) {
            this.peer.setRemoteDescription(new RTCSessionDescription(sdp)).then(function() {
                "function" == typeof setTemporaryLine && setTemporaryLine()
            })
        },
